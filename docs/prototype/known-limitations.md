...
|---|---|---|
| E-waste categories | The final SRS covers seven broad categories, not every product, component, brand, condition, or local trade name. | Show the suggested category, confidence, supported set, and a correction or unsupported-item path. |
...
| Classification | A model suggestion can be wrong or uncertain. The interim candidate gate is macro-F1 at least 0.70 with no category recall below 0.50; the operational low-confidence threshold still requires validation. | Keep human confirmation/correction and route low-confidence or unsupported cases without presenting a prediction as fact. Do not treat the candidate gate as evidence of field fitness. |
